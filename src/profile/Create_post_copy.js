...
import { View,Text,ImageBackground, Dimensions, Image, StyleSheet, TouchableOpacity, TextInput, Alert,  Portal, ScrollView, KeyboardAvoidingView, SafeAreaView, ActivityIndicator } from 'react-native';
...
const Createpost = ({navigation, route,profile_pic,user_id, value1,username, first_name, last_name}) => {
    ...
    const [displayCurrentAddress, setDisplayCurrentAddress] = useState('Wait, we are fetching you current location...');
    ...
    useEffect(() => {
        if(isCurrent){
            ...
            if(route.params?.data){
                ...
                setDescription(route.params.data.description.replace(/\\n/g,'\n'));
                setIsCurrent(false);
            }
        }
    ...
    const crpost_button= ()=>{
        if(description!='' && displayCurrentAddress!='' && displayCurrentAddress!="Wait, we are fetching you current location..." ){
        ...
        const userId = AsyncStorage.getItem('userid').then((value)=>{
            if(JSON.parse(value) != "" && JSON.parse(value) > 0 && JSON.parse(value) != null ){
                ...
                if(image != null && image != "" && image != undefined){
                    ...
                    formData.append('image', { uri: localUri, name: name, type: type});
                  }
        ...
        <View style={styles.container}>
            ...
            <View style={styles.container1}>
                <View style={{flexDirection:'row', justifyContent:'flex-start'}}>
                    <View style={{width:80, height:80, borderRadius:60, shadowOpacity:0.2, shadowOffset:{width:0, height:0} }}>
                    <Image style={{width:80, height:80, borderRadius:60, }}
                        //source={require('../../assets/icon/man.png')} 
                        source={{ uri:IMG_URL+profile_pic}} 
                    />
                    </View>
                    <Text style={{marginTop:10, fontSize:20}}>  {first_name} {last_name} </Text>
                </View>
            </View>
                ...
                <TextInput
                    multiline={true}
                    style={{marginHorizontal:10, marginBottom:20, borderWidth:1, borderColor:'#D7D7D7', borderRadius:10, height:160, width:'94%',textAlignVertical:"top", padding:10, }}
            ...
            <TouchableOpacity 
                onPress={()=>{pickImage(); }} 
                style={{flexDirection:'row', paddingBottom:2, marginHorizontal:10, borderBottomWidth:1}}>
            ...
            <TouchableOpacity  
                disabled={displayCurrentAddress!="Wait, we are fetching you current location..."? false:false}
                onPress={()=>alert_button()}
                style={{flexDirection:'row',  borderBottomWidth:1, marginHorizontal:10, marginTop:10, paddingBottom:2}}>


                <Entypo name="location-pin" size={26} color="#F9B7A7" />
                {displayCurrentAddress!="Wait, we are fetching you current location..."?
                        <Text numberOfLines={2} style={{marginLeft:10, width:'90%', fontSize:18}}>{displayCurrentAddress }</Text>
                        :
                        <Text style={{marginLeft:10, width:'90%', fontSize:18}}>Location</Text>
                }
            ...
            <TouchableOpacity style={{ marginLeft:10, marginRight:10, alignItems:'center', justifyContent:'center',  paddingVertical:10, borderRadius:20, marginTop:20, marginBottom:10, backgroundColor:'#B2C248'}} onPress={()=>crpost_button()}>
                    {isLoading?
                    <Text style={{textAlignVertical:'center'}}><ActivityIndicator color="#fff" size="small"  /></Text>
                :<Text style={{color:'#fff', fontSize:16, fontFamily:'font2'}}>Upload Post</Text>}
            ...
            <Overlayloading visible={loading} >Please wait...</Overlayloading>
